...
<div class="container-xxl my-4 vh-100">
    ...
    <div class="card p-2 pt-3">
        <form id="courseForm" action="{{ $formAction }}" method="POST">
            @csrf
            @if($formMethod === 'PUT')
            @method('PUT')
            @endif
            ...
            <div class="row mb-3 col-form-label col-form-label-top">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="title" name="title"
                        value="{{ old('title', $course->title ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>

            <div class="row mb-3 col-form-label">
                <label for="content" class="form-label-cus  col-sm-2">Mô tả: </label>
                <div class="col-sm-10">
                    <textarea id="content" name="content" class="tinymce" @if(!empty($readonly)) disabled
                        @endif>{{ old('content', $course->content ?? '') }}</textarea>
                </div>
            </div>
            <div class="row mb-3 col-form-label">
                ...
                <div class="col-sm-10">
                    ...
                    @endif
                    <a href="{{ route('admin.course.list') }}" class="btn btn-secondary">Quay lại</a>
                </div>
            </div>
