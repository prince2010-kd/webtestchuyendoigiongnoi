...
<div class="container-xxl my-4 vh-100">
    <h3>Danh sách danh mục trình độ học</h3>
    <div class="card p-2 pt-3">
        <div class="row mb-4 col-form-label col-form-label-top">
            <div class="d-flex justify-content-between">
                <a href="{{ route('admin.level.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="width: 5%;">{{ $lv->stt }}</td>
                    <td>{{ $lv->title }}</td>
                    <td style="vertical-align: middle;">{!! $lv->des !!}</td>
                    <td style="width: 10%;">
                        <div class="form-check form-switch">
                            <input class="form-check-input status-switch"
                                ...
                                role="switch"
                                data-id="{{ $lv->id }}"
                                @if ($lv->active == 1) checked @endif
                            />
                        </div>
                    </td>
                    <td style="width: 10%;">
                        <a href="{{ route('admin.level.edit', $lv->id) }}" class="btn btn-sm btn-info" title="Chỉnh sửa">
                            <i class="fa fa-edit"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-danger btn-delete" data-id="{{ $lv->id }}" title="Xóa">
        ...
        <div class="mt-4 pagination" id="pagination-post">
            @include('admin.component.pagination', ['posts' => $level])
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        $('.status-switch').change(function() {
            ...
            debounceTimeouts[id] = setTimeout(() => {
                ...
                $.ajax({
                    url: `/admin/list-level/updatestatus/${id}`,
                    method: 'PUT',
                    data: {
                        _token: '{{csrf_token()}}',
                        active: isActive
