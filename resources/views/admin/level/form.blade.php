...
<div class="container-xxl my-4 vh-100">
    ...
    <div class="card p-2 pt-3">
        <form id="levelForm" action="{{ $formAction }}" method="POST">
            @csrf
            @if($formMethod === 'PUT')
            @method('PUT')
            @endif
            ...
            <div class="row mb-3 col-form-label col-form-label-top">
                <label for="title" class="form-label-cus  col-sm-2">Tên trình độ</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="title" name="title"
                        value="{{ old('title', $level->title ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>

            <div class="row mb-3 col-form-label col-form-label-top">
                <label for="code" class="form-label-cus  col-sm-2">Mã trình độ</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="code" name="code"
                        value="{{ old('code', $level->code ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>
            ...
            <div class="row mb-3 col-form-label">
                ...
                <div class="col-sm-10">
                    <textarea id="des" name="des" class="tinymce" @if(!empty($readonly)) disabled
                        @endif>{{ old('des', $level->des ?? '') }}</textarea>
                </div>
            </div>
            <div class="row mb-3 col-form-label">
                ...
                <div class="col-sm-10">
                    ...
                    @endif
                    <a href="{{ route('admin.level.list') }}" class="btn btn-secondary">Quay lại</a>
                </div>
            </div>
