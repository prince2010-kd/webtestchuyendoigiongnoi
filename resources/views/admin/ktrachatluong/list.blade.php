...
@section('content')
    <div class="container-xxl my-4 vh-100">
        <h3>Danh sách đề kiểm tra chất lượng</h3>
        <div class="card p-2 pt-3">
            <div class="row mb-4">
                <div class="col-md-2 
                                    ">
                    <a href="{{ route('admin.quality.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
                </div>
            </div>
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container" style="overflow-x: auto; width: 100%;">
                    @include('admin.ktrachatluong.partials._table')
                </div>

                <div class="mt-4 pagination" id="pagination-post">
                    @include('admin.component.pagination', ['posts' => $items])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.btn-delete').click(function () {
                ...
                $.ajax({
                    url: '/admin/list-quality-check/' + id,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'DELETE'
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-quality-check/' + categoryId + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: newStt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-quality-check/' + categoryId + '/toggle-active',
                    type: 'put',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active
        ...
        $('#delete-selected').click(function (e) {
            ...
            if (ids.length === 1) {
                url = '/admin/list-quality-check/' + ids[0];
            } else {
                url = '{{ route("admin.quality.deleteMultiple") }}';
            }
        ...
        $(document).on('change', '.is-featured-toggle', function () {
            ...
            $.ajax({
                url: `/admin/list-quality-check/${postId}/toggle-is_featured`,
                type: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
