...
@section('content')
    <div class="container-xxl my-4 vh-100">
        <h3>Danh sách cấu hình SEO</h3>
        <div class="card p-2 pt-3">
            <div class="row mb-4">
                <div class="col-md-2 
                                    ">
                    <a href="{{ route('admin.cauhinhseo.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
                </div>
            </div>
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container" style="overflow-x: auto; width: 100%;">
                    @include('admin.cauhinhseo.partials._table')
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-seo/' + categoryId + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: newStt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-seo/' + categoryId + '/toggle-active',
                    type: 'put',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active
        ...
        $('#delete-selected').click(function (e) {
            ...
            if (ids.length === 1) {
                url = '/admin/list-seo/' + ids[0];
            } else {
                url = '{{ route("admin.cauhinhseo.deleteMultiple") }}';
            }
        ...
        $(document).on('change', '.is-featured-toggle', function () {
            ...
            $.ajax({
                url: `/admin/list-seo/${postId}/toggle-is_featured`,
                type: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
