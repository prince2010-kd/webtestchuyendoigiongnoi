        ...
        <div class="card p-2 pt-3">
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container">
                    @include('admin.category.partials._table')
                </div>

                <div class="mt-4 pagination" id="pagination-post">
                    @include('admin.component.pagination', ['posts' => $categories])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-category/' + id + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: stt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-category/' + id + '/toggle-active',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active,
            ...
            $('#delete-selected').click(function (e) {
                ...
                if (ids.length === 1) {
                    url = '/admin/list-category/' + ids[0];
                } else {
                    url = '{{ route("admin.category.deleteMultiple") }}';
                }
