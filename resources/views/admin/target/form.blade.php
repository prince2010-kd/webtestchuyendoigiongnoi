...
<div class="container-xxl my-4 vh-100">
    ...
    <div class="card p-2 pt-3">
        <form id="targetForm" action="{{ $formAction }}" method="POST">
            @csrf
            @if($formMethod === 'PUT')
            @method('PUT')
            @endif
            ...
            <div class="row mb-3 col-form-label col-form-label-top">
                <label for="title" class="form-label-cus  col-sm-2">Tên mục tiêu</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="title" name="title"
                        value="{{ old('title', $target->title ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>

            <div class="row mb-3 col-form-label col-form-label-top">
                <label for="code" class="form-label-cus  col-sm-2">Mã mục tiêu</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="code" name="code"
                        value="{{ old('code', $target->code ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>

            <div class="row mb-3 col-form-label col-form-label-top">
                <label for="price" class="form-label-cus  col-sm-2">Giá</label>
                <div class="col-sm-3">
                    <input type="number" class="form-control" id="price" name="price"
                        value="{{ old('price', $target->price ?? '') }}" required @if(!empty($readonly)) disabled @endif>
                </div>
            </div>

            <div class="row mb-3 col-form-label col-form-label-top">
                <label class="form-label-cus  col-sm-2"> Thuộc trình độ</label>
                <div class="col-sm-10">
                    <select name="levels[]" class="form-control" multiple>
                        @foreach($levels as $level)
                        <option value="{{ $level->id }}"
                            {{ isset($targetLevels) && in_array($level->id, $targetLevels) ? 'selected' : '' }}>
                            {{ $level->title }}
                        </option>
                        @endforeach
                    </select>
                    <small class="text-muted">Giữ Ctrl (hoặc Cmd trên Mac) để chọn nhiều trình độ</small>
                </div>
            </div>
            ...
            <div class="row mb-3 col-form-label">
                ...
                <div class="col-sm-10">
                    <textarea id="des" name="des" class="tinymce" @if(!empty($readonly)) disabled
                        @endif>{{ old('des', $target->des ?? '') }}</textarea>
                </div>
            </div>
            <div class="row mb-3 col-form-label">
                ...
                <div class="col-sm-10">
                    ...
                    @endif
                    <a href="{{ route('admin.target.list') }}" class="btn btn-secondary">Quay lại</a>
                </div>
            </div>
