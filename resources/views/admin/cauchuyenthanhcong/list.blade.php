...
@section('content')
    <div class="container-xxl my-4 vh-100">
        <h3>Danh sách câu chuyện thành công</h3>
        <div class="card p-2 pt-3">
            <div class="row mb-4">
                <div class="col-md-2">
                    <a href="{{ route('admin.thanhcong.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
                </div>
            </div>
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container" style="overflow-x: auto; width: 100%;">
                    @include('admin.cauchuyenthanhcong.partials._table')
                </div>

                <div class="mt-4 pagination" id="pagination-post">
                    @include('admin.component.pagination', ['posts' => $stories])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-success/' + id + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: stt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-success/' + id + '/toggle-active',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active,
            ...
            $('#delete-selected').click(function (e) {
                ...
                if (ids.length === 1) {
                    url = '/admin/list-success/' + ids[0];
                } else {
                    url = '{{ route("admin.thanhcong.deleteMultiple") }}';
                }
