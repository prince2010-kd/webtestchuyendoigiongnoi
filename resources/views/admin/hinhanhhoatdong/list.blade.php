...
@section('content')
    <div class="container-xxl my-4 vh-100">
        <h3>Danh sách hình ảnh hoạt động</h3>
        <div class="card p-2 pt-3">
            <div class="row mb-4">
                <div class="col-md-2">
                    <a href="{{ route('admin.hinhanhhdong.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
                </div>
            </div>
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container" style="overflow-x: auto; width: 100%;">
                    @include('admin.hinhanhhoatdong.partials._table')
                </div>

                <div class="mt-4 pagination" id="pagination-post">
                    @include('admin.component.pagination', ['posts' => $images])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-images/' + id + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: stt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-images/' + id + '/toggle-active',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active,
            ...
            $('#delete-selected').click(function (e) {
                ...
                if (ids.length === 1) {
                    url = '/admin/list-images/' + ids[0];
                } else {
                    url = '{{ route("admin.hinhanhhdong.deleteMultiple") }}';
                }
