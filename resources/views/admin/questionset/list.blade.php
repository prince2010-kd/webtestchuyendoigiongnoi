    ...
    <div class="container-xxl my-4 vh-100">
        ...
        <div class="card p-2 pt-3">
            <div class="row mb-4">
                <div class="col-md-2 
                                    ">
                    <a href="{{ route('admin.questionset.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
                </div>
            </div>
            ...
            <div class="ajax-pagination-container" data-table="#post-data" data-pagination="#pagination-post">
                <div id="table-container" style="overflow-x: auto; width: 100%;">
                    @include('admin.questionset.partials._table')
                </div>

                <div class="mt-4 pagination" id="pagination-post">
                    @include('admin.component.pagination', ['posts' => $sets])
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('change', '.stt-input', function () {
                ...
                $.ajax({
                    url: '/admin/list-questionset/' + categoryId + '/update-stt',
                    type: 'PUT',
                    data: {
                        _token: '{{ csrf_token() }}',
                        stt: newStt,
            ...
            $(document).on('change', '.active-toggle', function () {
                ...
                $.ajax({
                    url: '/admin/list-questionset/' + categoryId + '/toggle-active',
                    type: 'put',
                    data: {
                        _token: '{{ csrf_token() }}',
                        active: active
        ...
        $('#delete-selected').click(function (e) {
            ...
            if (ids.length === 1) {
                url = '/admin/list-questionset/' + ids[0];
            } else {
                url = '{{ route("admin.questionset.deleteMultiple") }}';
            }
        ...
        $(document).on('change', '.is-featured-toggle', function () {
            ...
            $.ajax({
                url: `/admin/list-questionset/${postId}/toggle-is_featured`,
                type: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
