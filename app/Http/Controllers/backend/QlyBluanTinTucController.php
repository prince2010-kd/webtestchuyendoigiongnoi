...
class QlyBluanTinTucController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tinTuc = $query->paginate($pageSize)->appends($request->all());

        // Trả về AJAX nếu cần
        if ($request->ajax()) {
            $table = view('admin.qlybluantintuc.partials._table', compact('tinTuc'))->render();
            $pagination = view('admin.component.pagination', ['posts' => $tinTuc])->render();
        ...
        return view('admin.qlybluantintuc.list', compact('tinTuc'));
    }

    public function comments(Request $request, $postId)
{
    $tinTuc = Posts::findOrFail($postId);

    $pageSize = $request->input('page_size', 100);

    $query = $tinTuc->comments()->orderByDesc('created_at');
    ...
    if ($request->ajax()) {
        $table = view('admin.qlybluantintuc.partials._tablecomments', compact('comments', 'tinTuc'))->render();
    ...
    return view('admin.qlybluantintuc.partials.comments', compact('tinTuc', 'comments'));
}
