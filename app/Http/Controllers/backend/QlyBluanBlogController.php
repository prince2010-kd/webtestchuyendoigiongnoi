...
class QlyBluanBlogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $blog = $query->paginate($pageSize)->appends($request->all());

        // Trả về AJAX nếu cần
        if ($request->ajax()) {
            $table = view('admin.qlybluanblog.partials._table', compact('blog'))->render();
            $pagination = view('admin.component.pagination', ['posts' => $blog])->render();
        ...
        return view('admin.qlybluanblog.list', compact('blog'));
    }

    public function comments(Request $request, $postId)
    {
        $blog = Posts::findOrFail($postId);

        $pageSize = $request->input('page_size', 10);

        $query = $blog->comments()->orderByDesc('created_at');
        ...
        if ($request->ajax()) {
            $table = view('admin.qlybluanblog.partials._tablecomments', compact('comments', 'blog'))->render();
        ...
        return view('admin.qlybluanblog.partials.comments', compact('blog', 'comments'));
    }
